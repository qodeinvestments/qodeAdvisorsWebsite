...
const FAQSection = () => {
  ...
  return (
    <div className="container mx-auto  py-16 graphik-font-regular bg-white">
      <div className="flex flex-col md:flex-row border border-black">
        <div className="md:w-1/3 border-r border-black p-8 ">
          <h2 className="text-6xl  transform -rotate-90 md:rotate-0">FAQ</h2>
        </div>
        <div className="md:w-2/3 p-8">
          {faqs.map((faq, index) => (
            <div key={index} className="mb-4 border border-black">
              ...
              {openAccordion === index && (
                <div className="p-4 text-3xl border-t border-black">
                  <p>{faq.answer}</p>
                </div>
              )}
            </div>
